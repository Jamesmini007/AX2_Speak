...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function saveTranslationSession() {
        ...
        // 번역 내용이 비어있으면 저장하지 않음
        const hasOriginal = translationContent.original && translationContent.original.length > 0;
        const hasTranslations = translationContent.translations && 
            Object.keys(translationContent.translations).some(langCode => 
                translationContent.translations[langCode] && 
                translationContent.translations[langCode].length > 0
            );
        
        if (!hasOriginal && !hasTranslations) {
            logger.log('번역 내용이 없어 저장하지 않습니다.');
            return null;
        }
